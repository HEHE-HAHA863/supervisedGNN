...
parser.add_argument('--filename_existing_gnn', nargs='?', const=1, type=str, default='gnn_J4_lyr20_Ntr50_num6000p=0.3_q=0.1')
...
def train_single(gnn, optimizer, gen, n_classes, it):
    ...
    labels = labels.type(dtype_l).to(device)  # 直接送到 GPU
...
def test_single(gnn, gen, n_classes, it):
    ...
    acc_test = original_compute_accuracy_multiclass(pred_single, labels, n_classes)

    elapsed = time.time() - start
    loss_value = loss_test.item()

    info = ['iter', 'avg loss', 'avg acc', 'edge_density', 'noise', 'model', 'elapsed']
    out = [it, loss_value, acc_test, args.edge_density, args.noise, 'GNN', elapsed]
    print(template1.format(*info))
    print(template2.format(*out))
...
if __name__ == '__main__':
    ...
    if (args.mode == 'test'):
        ...
        if ((filename != '') and (os.path.exists(path_plus_name))):
            ...
            print ('No such a gnn exists; creating a brand new one')
            filename = 'gnn_J' + str(args.J) + '_lyr' + str(args.num_layers) + '_Ntr' + str(args.N_train) + '_num' + str(args.num_examples_train) + 'p=' +str(args.p_SBM) + '_q=' + str(args.q_SBM)
            path_plus_name = os.path.join(args.path_gnn, filename)
            if (args.generative_model == 'SBM_multiclass'):
                gnn = GNN_multiclass(args.num_features, args.num_layers, args.J + 2, n_classes=args.n_classes)
